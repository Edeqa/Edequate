...
public class Splash extends AbstractAction<RequestWrapper> {
    ...
    public HtmlGenerator fetchSplash(boolean withButtons, boolean withRestore) {
        ...
        if(withRestore) {
            html.getHead().add(HtmlGenerator.SCRIPT).with(HtmlGenerator.ASYNC, true).with(HtmlGenerator.SRC, "/js/Edequate.js").with("data-variable", "u").with("data-callback", "u.require('/js/Restore.js', u).then(function(restore){restore.start()})").with("data-export-constants", true);
        } else {
